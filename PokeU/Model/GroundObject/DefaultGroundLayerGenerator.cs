    ...
    public class DefaultGroundLayerGenerator : ALandLayerGenerator
    {
        ...
        public override ILandLayer GenerateLandLayer(WorldGenerator worldGenerator, IntRect area, int minAltitude, int maxAltitude)
        {
            ...
            for (int i = 0; i < area.Height; i++)
            {
                ...
                {
                    this.GetLandType(altitudeLandLayerGenerator, area, i, j, out LandType landType, out LandType secondType, out LandTransition landTransition);
            ...
            out LandTransition landtransition)
        {
            ...
            for (int y = -1; y < 2; y++)
            {
                for (int x = -1; x < 2; x++)
                {
                    float power = altitudeLandLayerGenerato.GetPowerAt(new Vector2f(area.Left + j + x, area.Top + i + y));
