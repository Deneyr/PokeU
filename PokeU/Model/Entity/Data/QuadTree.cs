...
namespace PokeU.Model.Entity.Data
{
    ...
    {
        FloatRect bounds; // overall bounds we are indexing.
        Quadrant root;
        ...
        public void Insert(T node, FloatRect bounds)
        {
            if (this.bounds.Width == 0 || this.bounds.Height == 0)
            {
                throw new Exception("Empty canevas");
            }
            if (bounds.Width == 0 || bounds.Height == 0)
            {
                throw new Exception("Empty bounds");
            }
            if (this.root == null)
            {
        ...
        public IEnumerable<T> GetNodesInside(FloatRect bounds)
        {
        ...
        /// <param name="bounds">The bounds to test</param>
        /// <returns>List of zero or mode nodes found inside the given bounds</returns>
        public bool HasNodesInside(FloatRect bounds)
        {
            if (this.root == null)
            {
                return false;
        ...
        IEnumerable<QuadNode> GetNodes(FloatRect bounds)
        {
        ...
        internal class QuadNode
        {
            ...
            public QuadNode(T node, FloatRect bounds)
            {
                this.node = node;
                this.bounds = bounds;
            }
        ...
        {
            Quadrant parent;
            FloatRect bounds; // quadrant bounds.
            ...
            public Quadrant(Quadrant parent, FloatRect bounds)
            {
                this.parent = parent;
            ...
            /// <param name="bounds">The bounds of that node</param>
            /// <returns></returns>
            internal Quadrant Insert(T node, FloatRect bounds)
            {

                if (bounds.Width == 0 || bounds.Height == 0)
                {
                    throw new Exception("bounds empty");
                }
                ...
                while (true)
                {
                    ...
                    FloatRect topLeft = new FloatRect(toInsert.bounds.Left, toInsert.bounds.Top, w, h);
                    FloatRect topRight = new FloatRect(toInsert.bounds.Left + w, toInsert.bounds.Top, w, h);
                    FloatRect bottomLeft = new FloatRect(toInsert.bounds.Left, toInsert.bounds.Top + h, w, h);
                    FloatRect bottomRight = new FloatRect(toInsert.bounds.Left + w, toInsert.bounds.Top + h, w, h);

                    Quadrant child = null;

                    // See if any child quadrants completely contain this node.
                    if (topLeft.Contains(bounds.Left, bounds.Top) && topLeft.Contains(bounds.Left + bounds.Width, bounds.Top + bounds.Height))
                    {
                    ...
                    else if (topRight.Contains(bounds.Left, bounds.Top) && topRight.Contains(bounds.Left + bounds.Width, bounds.Top + bounds.Height))
                    {
                    ...
                    else if (bottomLeft.Contains(bounds.Left, bounds.Top) && bottomLeft.Contains(bounds.Left + bounds.Width, bounds.Top + bounds.Height))
                    {
                    ...
                    else if (bottomRight.Contains(bounds.Left, bounds.Top) && bottomRight.Contains(bounds.Left + bounds.Width, bounds.Top + bounds.Height))
                    {
            ...
            internal void GetIntersectingNodes(List<QuadNode> nodes, FloatRect bounds)
            {
                ...
                FloatRect topLeft = new FloatRect(this.bounds.Left, this.bounds.Top, w, h);
                FloatRect topRight = new FloatRect(this.bounds.Left + w, this.bounds.Top, w, h);
                FloatRect bottomLeft = new FloatRect(this.bounds.Left, this.bounds.Top + h, w, h);
                FloatRect bottomRight = new FloatRect(this.bounds.Left + w, this.bounds.Top + h, w, h);
            ...
            static void GetIntersectingNodes(QuadNode last, List<QuadNode> nodes, FloatRect bounds)
            {
                if (last != null)
                {
                    QuadNode n = last;
            ...
            internal bool HasIntersectingNodes(FloatRect bounds)
            {
            ...
            static bool HasIntersectingNodes(QuadNode last, FloatRect bounds)
            {
                if (last != null)
                {
                    QuadNode n = last;
